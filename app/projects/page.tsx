...
export default function ProjectsPage() {
  ...
  const select = (category: (typeof categories)[number]) => {
    if (category === active) return;
    setIsFiltering(true);
    if (timer.current) clearTimeout(timer.current);
    timer.current = setTimeout(() => { setActive(category); requestAnimationFrame(() => setIsFiltering(false)); }, 240);
  };
  ...
  return <>
    ...
    <main className="projects-page" id="top">
      ...
      <nav className="project-filters" aria-label="Filter projects by category">
        <Link href="/">Home</Link>
        {categories.map((category) => <button key={category} type="button" className={active === category ? "is-active" : ""} onClick={() => select(category)} aria-pressed={active === category}>{category}<span>{category === "All Work" ? projects.length : projects.filter((project) => project.category === category).length}</span></button>)}
      </nav>
      <div className="archive-status"><p><strong>{active}</strong> / {filtered.length} films</p><p>Select a title to enter the project.</p></div>
      <section className={`video-index${isFiltering ? " is-filtering" : ""}`} aria-label={`${active} projects`} aria-live="polite">
